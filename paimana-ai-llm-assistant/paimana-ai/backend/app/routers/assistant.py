...
def assistant_chat(req: ChatRequest):
    ...
    if gem_status.get("llm_enabled"):
        ...
        err = meta.get("error", "Call failed") if isinstance(meta, dict) else "Call failed"
        degrade_notice = f"LLM mode unavailable - using local assistant fallback ({err})."
    ...
    return ChatResponse(
        ...
        llm_enabled=False,
        notice=degrade_notice or gem_status.get("message", "Local assistant active"),
    )

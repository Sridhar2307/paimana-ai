...
function AppShell() {
  ...
  // Verify Supabase Connection on startup (non-blocking with timeout)
  useEffect(() => {
    let active = true;
    async function checkSupabase() {
      try {
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Supabase cloud service not reachable (operating in local mode)')), 2500)
        );
        ...
        if (active) {
          console.info('Operating in standalone mode with local PAIMANA-AI engine.');
        }
      }
    }
    checkSupabase();
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 bg-transparent">
          ...
          {activeTab === 'risk-analytics' && (
            <AnalyticsPage onNavigateToProjects={handleSearch} />
          )}
